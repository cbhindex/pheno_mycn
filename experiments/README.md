...
distribute, so they are **not** included in this repository (see `data/README.md`
for the expected formats). Each script documents the exact inputs it expects in
...
| `gmm_clustermap/` | `plot_clustermap.py`, `normalization_diagnostics.py` | Hierarchical clustering of all 189 slides by phenotype composition, with a MYCN-status colour strip. Drops the confirmed **artefact** component C4 (out-of-focus / section-edge / ink; see `C4_inspect/`), renormalises the five real phenotypes (C1,C2,C3,C5,C6), then **log-normalises + z-scores** them (transform chosen from the per-feature distribution / Q–Q panel in `normalization_diagnostics.py`) before clustering (Euclidean + Ward). Reads `gmm_responsibility/results/per_slide_stats.csv`; writes the clustermap, slide-clustering, MYCN-association (Fisher/permutation + effect sizes), subsample-stability, quality and confounder CSVs, and `SUMMARY.md`. Colours non-amp blue `#4F74C8`, MYCN-amp red `#D94F3D`. |
...
| `survival_analysis/` | `00_build_cohort.py` … `09_predicted_mycn_and_cox.py`, `_style.py` | Slide-level survival cohort build and exploratory Kaplan–Meier / log-rank analyses by MYCN status and by GMM component (Section 2.5). Run in numeric order. |
